...
export const getUserBasicInfoById = async (req, res) => {
  try {
    const { id } = req.params;

    // Find user by _id and select only firstname and company
    const user = await User.findById(id).select('firstname company');
